...
impl<T: MemAccess> JellyIntervalTimer<T> {
    ...
    pub fn set_enable(&self, enable: bool) {
        unsafe {
            self.reg_acc
                .write_reg_u8(REG_CONTROL, if enable { CONTROL_ENABLE } else { 0 });
        }
    }

    pub fn clear_counter(&self) {
        unsafe {
            let flag = self.reg_acc.read_reg_u8(REG_CONTROL) | CONTROL_CLEAR;
            self.reg_acc.write_reg_u8(REG_CONTROL, flag);
        }
    }

    pub fn set_compare_counter(&self, compare: u32) {
        unsafe {
            self.reg_acc.write_reg_u32(REG_COMPARE, compare);
        }
    }

    pub fn compare_counter(&self) -> u32 {
        unsafe { self.reg_acc.read_reg_u32(REG_COMPARE) }
    }

    pub fn counter(&self) -> u32 {
        unsafe { self.reg_acc.read_reg_u32(REG_COUNTER) }
    }

    pub fn polling_timer(&self) -> bool {
        unsafe { (self.reg_acc.read_reg_u8(REG_CONTROL) & CONTROL_IRQ) != 0 }
    }
